...
1. A random image to start with
2. A pre-trained network in evaluation mode
3. A nice way to access the resulting activations of any hidden layer we are interested in.
4. A loss function to compute the gradients and an optimizer to update the pixel values.
